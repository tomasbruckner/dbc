...
pub fn compute_layout(graph: &ErdGraph) -> DiagramLayout {
    // Defensive: an edge may reference a `TableKey` with no corresponding
    // node (e.g. FK metadata that omits schema while the table itself is
    // schema-qualified, so `FkRef{schema:None,..}` resolves to a key that
    // doesn't match the actual node's `Some(schema)` key). Drop such
    // dangling edges before any graph work runs, so cycle-detection and
    // layering only ever see structurally valid edges — mirrors
    // `export_svg`'s own "no matching TableInfo -> silently skipped"
    // defensive posture, and avoids a panic in `assign_layers`.
    let known: HashSet<&TableKey> = graph.nodes.iter().map(|n| &n.key).collect();
    let self_loops: Vec<&super::FkEdge> =
        graph.edges.iter().filter(|e| e.from == e.to && known.contains(&e.from)).collect();
    let plain_edges: Vec<&super::FkEdge> = graph
        .edges
        .iter()
        .filter(|e| e.from != e.to && known.contains(&e.from) && known.contains(&e.to))
        .collect();

    let touched: HashSet<&TableKey> =
        self_loops.iter().chain(plain_edges.iter()).flat_map(|e| [&e.from, &e.to]).collect();
    ...
    // Layering direction is REVERSED relative to `FkEdge`'s natural
    // (child -> parent) direction: a table with no outgoing FK (the
    // "root"/parent) must land at layer 0, so for layering purposes a
    // plain edge (from=child, to=parent) becomes (parent, child) —
    // "parent" is the layering-graph source. A back edge (found by DFS in
    // the NATURAL direction) is left in its natural (from, to) orientation
    // here so the net effect breaks the cycle in the reversed graph too.
