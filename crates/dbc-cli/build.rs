//! Windows-only: version resource and icon for `dbc.exe`. Same script as
//! `crates/dbc-ui/build.rs`, which explains why; the short version is
...
fn main() {
    ...
    res.set("FileDescription", "dbc — příkazová řádka");
    res.set("CompanyName", "Tomáš Bruckner");
    res.set("LegalCopyright", "© Tomáš Bruckner");
    res.set("OriginalFilename", "dbc.exe");
